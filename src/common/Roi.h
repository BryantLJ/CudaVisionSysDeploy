 ...
 * TYPE DEFINITIONS
 *****************************************************************************/
enum classType {CT_Unknown=-2, CT_Background=-1, CT_Pedestrian=1, CT_Car=2, CT_Van=3, CT_Truck=4, CT_Bike=5, CT_Group=6, CT_TSignForbiden=7, CT_TSignWarning=8, CT_TSignInfoRect=9, CT_TSignInfo=10, CT_TSignStop=11, CT_TSignGiveAway=12, CT_TSignMandatory=13, CT_TSignForbiden2=14};
...
struct RoiDisplayParam
{
	...
	bool	m_showClassName;	///< Show the class name
	bool	m_showClassImage;	///< Show the class name
...
public:
	...
	CRoi():
		...
		m_label(0), 
		m_confidence(0),									// Label and confidence
	...
	CRoi(const int x, const int y, const int w, const int h, const float conf=0.0f):
		...
		m_xw(0), m_yw(0), m_zw(0), m_ww(0), m_hw(0), m_dw(0),			// World coordinates (Stereo)
		m_label(0), 
		m_confidence(conf),									// Label and confidence
		m_obligatory(true),												// Indicates if it is mandatory to detect (For annotations)
		m_stdDev(-1.0f),												// StdDev of the ROI
		m_class(CT_Unknown), m_view(VT_Unknown), 
		m_visibility(VBT_Unknown), m_scene(ST_Unknown), m_pose(PT_Unknown),
	...
	void Reset()
	{
		...
		m_label=0;
		m_confidence=0.0f;					// Label and confidence
		m_obligatory = true;							// Indicates if it is mandatory to detect (For annotations)
		m_stdDev = -1.0f;								// StdDev of the ROI
		m_class=CT_Unknown; m_view=VT_Unknown; 
		m_visibility=VBT_Unknown; m_scene=ST_Unknown; m_pose=PT_Unknown;
	...
	void Copy(const CRoi& roi)
	{
		...
		m_confidence=roi.confidence(); 
		m_label = roi.label();	
		m_stdDev=roi.stdDev(); m_obligatory = roi.obligatory();
	...
	classType&		Class			()			{ return m_class;		}
